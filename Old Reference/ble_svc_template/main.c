...
static simple_ble_char_t my_char = {.uuid16 = 0x108b};
static char str_printed[24] = "Testing";
...
void ble_evt_write(ble_evt_t const* p_ble_evt) {
    ...
    if (simple_ble_is_char_event(p_ble_evt, &my_char)) {
      printf("Got to writing to screen\n");
      /*if (str_printed) {
        display_write("TRUE", DISPLAY_LINE_0);
      } else {
        display_write("FALSE", DISPLAY_LINE_0);
      }*/
      display_write(str_printed, DISPLAY_LINE_0);
    }
}
...
int main(void) {
  ...
  simple_ble_add_characteristic(1, 1, 0, 0,
      sizeof(str_printed), (uint8_t*)&str_printed,
      &led_service, &my_char);
